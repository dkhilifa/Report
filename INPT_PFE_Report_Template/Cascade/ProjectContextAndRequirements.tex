\chapter{Project Context and Requirements}
...
``STMicroelectronics'' was officially adopted in May 1998~\cite{st-about}. Figure~\ref{fig:st-logo}
...
Benchmark} --- was scheduled at STMicroelectronics Tunis from February to September 2026.
At the date of writing, 24~August 2026, this project window was still in progress.
...
The first experiments exposed recurring work in linker-map reconciliation, project
preparation, and evidence tracking. We therefore developed two supportive tools after the
USB pilot to make the established workflow more systematic:
...
(detailed in Chapter~\ref{chap:footprint-tool}).
...
equivalence (detailed in Chapter~\ref{chap:benchmark-agent}).
\end{itemize}
...
executing the code. The measurement is reproducible when the source, toolchain, build
options, and build environment are fixed and recorded.
...
\begin{enumerate}
\item \textbf{Task~1 --- Board-equivalence assignment}: for a competitor-board list
provided by the host team, identify and document the nearest STM32 board according to
processor, memory, peripheral, board, and product-positioning characteristics. This was a
standalone deliverable and did not determine the benchmark platforms.
\item \textbf{Task~2 --- Literature and ecosystem study}: research the STM32Cube
ecosystem, competing vendor ecosystems (GD32, NXP, TI, Renesas), linker-map formats,
and existing benchmarking practices.
\item \textbf{Task~3 --- Benchmark scenario design}: define the software stacks to
...
\item \textbf{Task~4 --- Benchmark execution and porting}: build and validate each
...
\item \textbf{Task~5 --- Result comparison and interpretation}: compare per-layer
...
\item \textbf{Task~6 --- Supportive tool development}: design and implement
...
\item \textbf{Task~7 --- Documentation and report}: document the methodology, tools, and
...
improve. The theoretical foundations --- the STM32Cube ecosystem, firmware footprint, and
linker maps --- provide the vocabulary for the chapters that follow. We first document the
standalone board-equivalence assignment. The report then starts the independent benchmark
work with the pilot USB campaign. The limitations observed during that pilot motivate the
supportive tools, before the later GD32 and TI campaigns extend the method to broader
scenarios.
