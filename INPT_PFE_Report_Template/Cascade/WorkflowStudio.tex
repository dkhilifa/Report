...
The USB pilot in Chapter~\ref{chap:cascade-usb-pilot} showed that a defensible comparison
required repeated reconciliation of linker-map symbols, software layers, and measurement
boundaries. It also showed why each result needed traceable source artifacts when summary
and detail sheets did not agree. These limitations defined the first supporting-tool
requirements.
...
\reportscreenshot
  {Figures/footprint-tool/screenshots/main-analysis-window.png}
  {Open MCU Workflow Studio with a completed two-map comparison. Capture the full application window with the project summary, one footprint chart, the analysis tabs, and mapping status visible.}
  {Main MCU Workflow Studio analysis window after a cross-vendor footprint run.}
  {fig:tool-screenshot-main}
...
\caption{Relative weights of the signals combined by the deterministic mapping scorer. Role
and alias similarity carry the largest individual weights; the optional LLM-similarity signal (highlighted) is capped so
...
\caption{Detailed signal computation and penalty system. The deterministic evidence signals
are combined with knowledge-base priors for each candidate pair; penalties for size mismatch and layer conflict reduce the composite
...
\reportscreenshot
  {Figures/footprint-tool/screenshots/mapping-review-queue.png}
  {Open the mapping review queue for a real comparison. Show several candidate pairs, confidence values, acceptance states, and the per-signal explanation for the selected pair.}
  {Mapping review queue and confidence diagnostics for a low-confidence symbol pair.}
  {fig:tool-screenshot-mapping-review}
...
The tool is designed so that every benchmark run emits a \emph{run manifest} that captures the tool version, the
deterministic mode (strict or assisted), the schema version, input file checksums, and a
hash of the equivalence database state at the time of execution. Together with stable sort
tie-breaks, rounded scores, and canonical export ordering, this information is intended to
make a run traceable and reduce output variation when frozen inputs and configuration are
processed again. These mechanisms provide a basis for reproducibility testing; however,
byte-for-byte equivalence across environments has not yet been established by a dedicated
validation campaign.
...
\reportscreenshot
  {Figures/footprint-tool/screenshots/exported-results.png}
  {Open a generated benchmark workbook or PDF report. Capture the global flash/RAM comparison, one per-layer breakdown, and the verdict or quality summary in a single legible view.}
  {Generated footprint report showing the global and per-layer comparison outputs.}
  {fig:tool-screenshot-export}

\section{Engineering value and limitations}
\label{sec:tool-value}

MCU Workflow Studio consolidates operations that would otherwise require manual inspection
of two linker maps: symbol extraction, architectural classification, candidate mapping,
footprint aggregation, and export. Its review queue and per-match diagnostics preserve the
points that still require engineering judgement, while the run manifest records the inputs
and knowledge-base state used for a comparison. The tool therefore provides a more structured
and traceable analysis process, although this report does not quantify the time saved or
establish that every mapping is correct.

The twelve-signal scoring engine and the cross-vendor equivalence database allow the tool to
propose mappings without requiring language-model assistance. Their presence should not be
confused with measured mapping accuracy: a labelled validation corpus covering all supported
vendors has not yet been reported. Likewise, firmware quality grades and
optimisation-potential estimates are heuristic indicators intended to guide inspection, not
independently validated measurements of software quality.
...
We designed and built MCU Workflow Studio to structure cross-vendor footprint analysis. Its
core parses two IAR linker maps, classifies symbols into architectural layers, proposes
cross-vendor correspondences, and reports per-layer and global flash and RAM differences
with a diagnostic breakdown. The run manifest records the inputs and configuration needed
for traceability and later reproducibility testing. Firmware quality metrics and optional
language-model assistance support review, but neither replaces validation against labelled
data and source artifacts. The chapters that follow use the tool's footprint outputs under
these limitations.
