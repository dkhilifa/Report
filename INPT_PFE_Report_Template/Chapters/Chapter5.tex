...
\reportscreenshot
      {Figures/ti-static/screenshots/analysis-reports.png}
      {Open the generated static-analysis output for the pinned ST and TI SDK releases. Capture the tool and version metadata plus representative cloc, Lizard, Cppcheck MISRA, and jscpd results, using a report dashboard or a clean terminal collage.}
      {Static-analysis evidence generated for the STM32CubeC5 LL and MSPM33 driverlib source sets.}
      {fig:ti-screenshot-static-analysis}
...
\reportscreenshot
      {Figures/ti-freertos/screenshots/project-configuration.png}
      {Capture the IAR project options or generated configuration for both FreeRTOS targets. Make the device, optimisation level, core clock, FreeRTOS version, heap size, tick rate, and key scheduler options legible.}
      {Recorded build and FreeRTOS configuration for the STM32C5 and MSPM33 targets.}
      {fig:ti-screenshot-freertos-config}
...
All six images were rebuilt with IAR EWARM~9.60.3 using high optimisation for size
(\texttt{CCOptLevel=3}, \texttt{CCOptStrategy=1}). Flash is read-only code plus read-only data;
RAM is read-write data, following the linker-map accounting used in
