...
MCU Workflow Studio addresses the analysis problems exposed by the pilot, but producing
comparable builds remained a separate source of manual effort. The previous chapter
presented a tool that analyses the \emph{output} of a benchmark ---
...
is designed to assist end-to-end benchmark campaigns: from scenario resolution and
...
github-copilot}. Within the permissions and materials available to a campaign, the agent
can execute builds, fetch missing
materials from official vendor sources, apply fixes, and iterate --- asking the user
only before programming physical hardware. Where MCU Workflow Studio analyses what
\emph{exists}, the agent system assists the creation and repair of missing projects under
rules intended to preserve semantic equivalence and benchmark fairness. The two tools are
complementary: the agent prepares candidate builds and evidence, and the studio analyses
their footprint after engineering review.
...
The coordinator uses a deterministic routing tree. Project understanding goes to
\emph{benchmark-analysis}, cross-vendor porting to \emph{Creation-Porting}, and build
execution and configuration to \emph{Build-Project}. Failures accompanied by real evidence
go to \emph{Debugger}, while result interpretation uses its dedicated skill. This routing
keeps each task within the configured specialist's scope and limits the context passed to it.
...
Every benchmark campaign is intended to follow the same twelve-step sequence. This stable
ordering makes the expected inputs, decisions, approval gate, and outputs explicit. It
supports consistent execution and later audit, but does not by itself prove that repeated
agent sessions will produce identical projects or measurements.
...
\caption{Designed end-to-end benchmark lifecycle. The MCU Benchmark Copilot Agent is configured to handle steps~1
through~11, subject to a hardware gate at step~9; once semantic equivalence is
confirmed, the validated linker map is handed to MCU Workflow Studio for footprint analysis.}
\label{fig:agent-workflow}
\end{figure}

\reportscreenshot
	{Figures/benchmark-agent/screenshots/copilot-campaign.png}
	{In VS Code, open Copilot Chat on a real benchmark request. Capture the user prompt, the selected benchmark agent, at least one delegated action, and a concise tool or build result. Hide personal paths, tokens, and unrelated conversations.}
	{Benchmark campaign initiated through the custom agent in VS Code Copilot Chat.}
	{fig:agent-screenshot-campaign}

\reportscreenshot
	{Figures/benchmark-agent/screenshots/iar-build-output.png}
	{Build a representative benchmark in IAR EWARM. Capture the project tree and Build Output pane with the target, configuration, warning or error count, and successful build summary visible.}
	{IAR EWARM build evidence produced during an agent-assisted benchmark campaign.}
	{fig:agent-screenshot-iar-build}
...
\path{.github/benchmark/campaigns/<campaign_id>/}. Key artifacts include:
...
\reportscreenshot
	{Figures/benchmark-agent/screenshots/campaign-dossier.png}
	{In VS Code Explorer, open one completed campaign dossier. Show the manifest, execution status, fairness review, semantic-equivalence record, and measurement metadata; open one file so its populated fields are legible.}
	{Structured campaign dossier retained for traceability and fairness review.}
	{fig:agent-screenshot-dossier}

This artifact model improves campaign auditability and records the inputs needed for later
reproduction. A reviewer can inspect
...
\reportscreenshot
	{Figures/benchmark-agent/screenshots/workspace-configuration.png}
	{In VS Code, show the .github agents, skills, tools, config, prompts, and campaign folders in Explorer. Open a populated board or toolchain YAML entry in the editor.}
	{Agent-system workspace and a board configuration registry entry in VS Code.}
	{fig:agent-screenshot-workspace}
...
The agent system encodes the benchmark methodology in reusable policies, checklists,
specialist roles, and structured campaign artifacts. This design makes semantic equivalence,
fairness, provenance, baseline immutability, and unresolved unknowns explicit during a
campaign. It can assist with source discovery, project preparation, build execution, and
diagnostic interpretation while preserving a hardware-approval gate before operations on a
physical board.

The structured outputs make completed actions and remaining blockers easier to inspect.
However, this report does not contain a controlled productivity study comparing agent-assisted
and manual porting, nor does it demonstrate that every campaign can proceed without human
intervention. We therefore do not quantify time savings or claim autonomous completion as a
general result. Reproducibility also depends on the model version, available tools, source
packages, configuration state, and hardware observations; the campaign dossier improves
traceability but cannot remove these dependencies.
...
We designed and built the MCU Benchmark Copilot Agent as a structured assistant for benchmark
campaigns. Its specialist agents, reusable skills, automation scripts, and configuration
registry organise the path from scenario resolution to evidence review and reduce unsupported
hardware assumptions. Shared policies expose semantic-equivalence checks, fairness risks,
baseline changes, provenance, and unresolved unknowns rather than treating an unverified build
as a completed benchmark.

Together with MCU Workflow Studio, the agent supports two complementary parts of the method:
project preparation and traceability on one side, and linker-map footprint analysis on the
other. The system records information needed for later reproduction, but its productivity,
repeatability, and end-to-end autonomy still require evaluation through recorded builds and
hardware-backed benchmark runs.
